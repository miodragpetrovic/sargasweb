...
// -----------------------------------------------------------------------------
// Komponenta sa GSAP animacijama
// -----------------------------------------------------------------------------
...
export default function PreiseClient() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Karten – Projekt-Pakete
      gsap.utils.toArray<HTMLElement>(".preise-card").forEach((card, i) => {
        gsap.from(card, {
          y: 40,
          opacity: 0,
          duration: 0.7,
      ...
      // Karten – Betreuung
      gsap.utils.toArray<HTMLElement>(".care-card").forEach((card, i) => {
        gsap.from(card, {
          y: 30,
          opacity: 0,
          duration: 0.6,
  ...
  return (
    <main
      ref={rootRef}
      className="min-h-screen bg-slate-950 text-slate-100 pt-5"
    >
      <section className="border-b border-slate-800/60 bg-gradient-to-b from-slate-950 via-slate-950 to-slate-950/90">
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <div className="max-w-2xl">
            <span className="preise-hero-badge inline-flex items-center rounded-full border border-sky-500/40 bg-sky-500/10 px-3 py-1 text-xs font-medium uppercase tracking-[0.18em] text-sky-300">
              Preise
            </span>
            <h1 className="preise-hero-title mt-4 text-3xl font-semibold tracking-tight text-slate-50 md:text-4xl">
              Faire Preise für moderne Weblösungen
            </h1>
            <p className="preise-hero-text mt-3 text-sm md:text-base text-slate-300">
            ...
            <p className="preise-hero-text mt-3 text-xs text-slate-400">
              Individuelle Wünsche und komplexere Projekte kalkulieren wir
              gerne in einem persönlichen Angebot.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-10 md:py-14">
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-2">
          {projectPackages.map((pkg) => (
            <div
              key={pkg.title}
              className="preise-card group flex flex-col justify-between rounded-2xl border border-slate-800 bg-slate-950/60 p-5 shadow-[0_18px_40px_rgba(0,0,0,0.6)] transition-transform hover:-translate-y-1 hover:border-sky-500/50 hover:shadow-[0_22px_50px_rgba(8,47,73,0.65)]"
            >
              <div>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-16 md:pb-20">
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-3">
          {carePackages.map((pkg) => (
            <div
              key={pkg.title}
              className="care-card flex flex-col rounded-2xl border border-slate-800 bg-slate-950/70 p-5 shadow-[0_18px_40px_rgba(0,0,0,0.6)]"
            >
              <div>
